...
1. 숫자 버튼 채널 변경
  완료 1-1. 한자리 수 입력 + 확인 -> 채널 변경
  완료 1-2. 두자리 수 입력 -> 자동 변경
  완료 1-3. 세자리 수 입력
      : 2두자리수 입력으로 동작 혹은 1자리 수 입력으로 동작
  완료 1-4. '0'으로 시작 -> 1자리수로 인식

2. 선호 채널
   2-1. 선호 채널 추가
      : 선호 채널에 없으면 저장... 신규면 추가

3. 선호 채널 이동
   3-1. 채널 이동 : 다음 채널 중 최소 값
            단, 최대값이 없으면 선호 채널 중 최소값으로 이동

4. 채널 검색
   4-1. Mock으로 setCH 함수 구현......고민할 것..

5. 업/다운 동작 (저장 채널이 없으면..)
   5-1. 채널 업이면 +1.. 최대값보다 커지만..0으로 이동
   5-2. 채널 다운이면 -1.. 최소값보다 작아지만 99로 이동

6. 업/다운 동작 (저장 채널이 있으면..)
   5번의 전체 채널 메모리를 -> 저장 채널 메모리로 변경하면 될 듯...
